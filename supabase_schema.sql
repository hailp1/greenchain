...
-- Users can view all entities (directory) but only update their own
CREATE POLICY "Public can view entities" ON entities FOR SELECT USING (true);
CREATE POLICY "Entities can update own profile" ON entities FOR UPDATE USING (auth.uid()::text = wallet_address);
...
USING (entity_id IN (SELECT id FROM entities WHERE wallet_address = auth.uid()::text));
...
USING (EXISTS (SELECT 1 FROM entities WHERE wallet_address = auth.uid()::text AND role = 'ADMIN'));
